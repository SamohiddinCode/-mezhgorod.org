...
// $filter_mini_np = "'п.', 'д.', 'ст', 'нп', 'дп', 'ж/д_оп', 'ж/д_рзд', 'ж/д_ст', 'снт', 'х', 'ж/д_платф', 'г-к', 'рзд', 'массив', 'тер', 'с/с'";
$ignore_oblast = "'1', '1', 'Алтай Республика', 'Алтайский край','Амурская область', 'Архангельская область', 'Еврейская Аобл', 'Забайкальский край', 'Камчатский край', 'Карелия Республика', 'Кемеровская область', 'Магаданская область', 'Мурманская область', 'Ненецкий АО', 'Хакасия Республика','Ямало-Ненецкий АО', 'Чукотский АО', 'Хабаровский край', 'Тыва Республика', 'Таймырский (Долгано-Ненецкий) АО', 'Сахалинская область', 'Саха (Якутия) Республика', 'Приморский край'";
//id_obl
...
$localff = [];
$firms = [];
$title_gen = 'Междугороднее такси по России и СНГ';
$linea_view = '';
$check_last_obl;
...
if ($arr_ccc==0) {
	...
	$sql = "SELECT * FROM start_city WHERE name_obl_rus NOT IN($ignore_oblast) ORDER BY name_obl_rus";
	...
	foreach($city_aa as $element) {
		if(!empty($element)){
			if ($element[0] == $this_page) {
				$localaa = $element;
				$h1_title = "Междугороднее такси по России и СНГ ".$element[4].' ('.$element[2].')';
				$findstep1++;
			}
		}
	};
...
if ($h1_title=='') {$h1_title = 'Междугороднее такси по России и СНГ';};
...
?>
	...
	<section class="section3" style="background: #f6f7f8;">
		<div class="container">
			...
			<?php if ($arr_ccc==1) {
				if ($findstep1 > 0) { ?>
					<h2>Выберите город куда необходимо приехать</h2>
